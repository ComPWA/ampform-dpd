...
def describe_formulate_dynamics():
    def it_formulates_a_generic_function():
    ...
    def it_accepts_published_notation():
        model = ModelDefinition(
            distributions=[],
            functions=[
                GenericFunctionDefinition(
    ...
    def it_formulates_bugg_lineshapes(model_definition: ModelDefinition, chain_id: int):
        chain = get_decay_chains(model_definition)[chain_id]

        dynamics = formulate_dynamics(chain, model_definition)

        assert dynamics.expression.free_symbols == {
            sp.Symbol("sigma1", nonnegative=True)
        }

    def it_uses_complex_phase_space_for_multichannel_dynamics(
...
def describe_formulate_form_factor():
    def it_formulates_a_momentum_power():
        model = ModelDefinition(
            distributions=[],
            functions=[
                MomentumPowerDefinition(name="momentum", type="MomentumPower", l=3)
            ],
            domains=[],
            misc={},
            parameter_points=[],
    ...
    def it_uses_serialized_normalization(
        model_definition: ModelDefinition,
    ):
...
def describe_formulate_polynomial():
    def it_formulates_a_polynomial():
    ...
    def it_rejects_a_variable_mismatch():
