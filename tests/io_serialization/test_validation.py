...
def describe_validate_checksums():
    @pytest.mark.parametrize("backend", ["numpy", "jax"])
    def it_validates_real_and_complex_checksums(
    ...
    def it_respects_the_tolerance_boundary(model_definition: ModelDefinition):
        workspace = load_workspace(model_definition)
        checksum = next(
    ...
    def it_uses_the_symbolic_variable_of_a_named_function(
    ...
    def it_diagnoses_a_missing_target(model_definition: ModelDefinition):
        workspace = load_workspace(model_definition)
        checksum = next(
            item for item in workspace.checksums if item["distribution"] == "L1600_BW"
        )
    ...
    def it_continues_after_an_invalid_point(
        model_definition: ModelDefinition,
    ):
        workspace = load_workspace(model_definition)
    ...
    def it_diagnoses_an_invalid_reference(model_definition: ModelDefinition):
        workspace = load_workspace(model_definition)
        checksum = next(
            item for item in workspace.checksums if item["distribution"] == "L1600_BW"
        )
    ...
    def it_diagnoses_non_finite_output(model_definition: ModelDefinition):
        workspace = load_workspace(model_definition)
        checksum = next(
            item for item in workspace.checksums if item["distribution"] == "L1600_BW"
        )
    ...
    def it_diagnoses_an_invalid_evaluation(model_definition: ModelDefinition):
        workspace = load_workspace(model_definition)
        checksum = next(
            item for item in workspace.checksums if item["distribution"] == "L1600_BW"
        )
    ...
    def it_continues_after_a_compilation_failure(
        model_definition: ModelDefinition,
