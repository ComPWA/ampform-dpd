...
def describe_MultichannelBreitWigner():
    def it_matches_reference_implementation():
        s = 2.0
        mass = 1.4051
        channel_definitions = [
            {
    ...
    def it_equals_single_channel_breit_wigner():
        ...
        breit_wigner = BreitWigner(
            s,  # ty: ignore[invalid-argument-type]
            mass,  # ty: ignore[invalid-argument-type]
            width,  # ty: ignore[invalid-argument-type]
            m1,  # ty: ignore[invalid-argument-type]
            m2,  # ty: ignore[invalid-argument-type]
            angular_momentum,
            meson_radius,  # ty: ignore[invalid-argument-type]
    ...
    def it_matches_two_channel_flatte_reference():
    ...
    def it_continues_below_threshold(s: sp.Expr, expected: complex):
    ...
    def it_is_continuous_at_imaginary_epsilon():
        """https://github.com/mmikhasenko/HadronicLineshapes.jl/blob/78f1e04/test/test-lineshapes.jl#L38-L43"""
        s = sp.Float(0.54**2)
